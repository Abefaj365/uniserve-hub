...
import { FileText, Calendar, User, Building2, Tag, Clock, Paperclip, Star } from "lucide-react";
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="flex items-center gap-2 text-sm font-semibold">
                      ...
                      {existingFeedback ? "Your Feedback" : "Rate this Resolution"}
                    </h3>
                    {existingFeedback && (
                      <div className="flex items-center gap-1 mt-1">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star
                            key={s}
                            className={`h-4 w-4 ${s <= (existingFeedback.rating ?? 0) ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground/30"}`}
                          />
                        ))}
                        {existingFeedback.comment && (
                          <span className="text-xs text-muted-foreground ml-2">"{existingFeedback.comment}"</span>
                        )}
                      </div>
                    )}
                  </div>
